...
func (e *DisksExporter) Collect(ch chan<- prometheus.Metric) {
	...
	for _, s := range disks {
                {
                        g := e.Online.WithLabelValues(s.MountPath, s.HostName, string(s.Location), s.DiskStatus)
			g.Set(float64(s.Online))
                        g.Collect(ch)

                        g = e.DiskSize.WithLabelValues(s.MountPath, s.HostName, string(s.Location), s.DiskStatus)
		...
		for i, k := range e.UsageStats {
			...
			g := k.WithLabelValues(s.MountPath, s.HostName, string(s.Location), s.DiskStatus)
			g.Set(v)
			g.Collect(ch)
		}
